...
```bash
rusty-sam-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
rusty-sam-app$ sam local start-api
rusty-sam-app$ curl http://localhost:3000/
```
...
```bash
rusty-sam-app$ sam logs -n HelloWorldFunction --stack-name rusty-sam-app --tail
```
